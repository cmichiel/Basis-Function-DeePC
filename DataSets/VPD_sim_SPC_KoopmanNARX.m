...
PhiL = [];
PhiL = [PhiL rbf_before_out_layer_norm(centers_sigmasL,u_ini,uf1,y_ini,data_mean_1,data_std_1,Basis_funcKoopman)];


PhiNL = [];
PhiNL = [PhiNL rbf_before_out_layer_MIXED(centers_sigmasNL,u_ini,uf,y_ini,data_mean,data_std,Basis_funcNARX)];
PhiPart = [PhiL;uf1; PhiNL];
Phi = [Phi,PhiPart];
out_vector = [];
for j = 1:(N)
...
constraints = [y == Theta*[nl_part;(u-data_mean_2')./data_std_2';
    rbf_before_out_layer_MIXED(centers_sigmasNL,u_ini_var,u,y_ini_var,data_mean,data_std,Basis_funcNARX) ]];
...
title ('SPC: Reference vs Closed Loop Output');
grid on
%saveas(fig,'RBF-SPC\Figures\Closed Loop Output\KoopmanNARX-SPC\VDP_KoopmanNARX-RBF_SINREF-SPC_output_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_n_basisNARX'+string(n_basisNARX)+'_Tini'+string(Tini)+'_KoopmanBasis'+Basis_funcKoopman+'_NARXBasis'+Basis_funcNARX+'.png')
...
grid on
xlabel('Iterations');
%saveas(fig2,'RBF-SPC\Figures\Control Input\KoopmanNARX-SPC\VDP_KoopmanNARX-RBF_SINREF-SPC_input_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_n_basisNARX'+string(n_basisNARX)+'_Tini'+string(Tini)+'_KoopmanBasis'+Basis_funcKoopman+'_NARXBasis'+Basis_funcNARX+'.png')
...
grid on;
xlabel('Iterations');
%saveas(fig3,'RBF-SPC\Figures\Tracking Error\KoopmanNARX-SPC\VDP_KoopmanNARX-RBF_SINREF-SPC_Error_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_n_basisNARX'+string(n_basisNARX)+'_Tini'+string(Tini)+'_KoopmanBasis'+Basis_funcKoopman+'_NARXBasis'+Basis_funcNARX+'.png')
...
save('Mircea-Plotting/Data/u_dataKoopmanNARXSPC_SINREF_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_n_basisNARX'+string(n_basisNARX)+'_Tini'+string(Tini)+'_KoopmanBasis'+Basis_funcKoopman+'_NARXBasis'+Basis_funcNARX+'.mat',"uvec")
save('Mircea-Plotting/Data/y_dataKoopmanNARXSPC_SINREF_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_n_basisNARX'+string(n_basisNARX)+'_Tini'+string(Tini)+'_KoopmanBasis'+Basis_funcKoopman+'_NARXBasis'+Basis_funcNARX+'.mat',"y_data")
save('PredictionSets/YkSPC_KoopmanNARX_KoopmanNARXSPC_STEPREF_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_n_basisNARX'+string(n_basisNARX)+'_Tini'+string(Tini)+'_KoopmanBasis'+Basis_funcKoopman+'_NARXBasis'+Basis_funcNARX+'.mat',"Yk")