...
file_string = 'RBFs/'+string(Basis_funcKoopman)+'/'+string(Basis_funcNARX)+ '/RBF_Params_KoopmanNARXResnet_Tini'+string(Tini)+'_nbasisKoopman'+string(n_basisKoopman)+'_nbasisFullBasis'+string(n_basisNARX)+'_N'+string(N)+'_KoopmanBasis_'+string(Basis_funcKoopman)+'_FullBasis_'+string(Basis_funcNARX)+'.mat';
...
%% recompute Theta 
load('DataSets/u_data20sinsSweptAmpRed.mat')
load('DataSets/y_data20sinsSweptAmpRed.mat')
...
Phi_part = [ rbf_before_out_layer_norm(centers_sigmasKoopman,u_ini,uf1,y_ini,data_mean_1,data_std_1,Basis_funcKoopman); rbf_before_out_layer_MIXED(centers_sigmasNL,u_ini,uf,y_ini,data_mean,data_std,Basis_funcNARX)  ;ResPart];
Phi = [Phi Phi_part];
out_vector = [];
for j = 1:(N)
...
grid on
title('T-SNE Clustering of \Phi for KoopmanNARXResnet Basis')
...
constraints = [y == Theta*[nl_part;
    rbf_before_out_layer_MIXED(centers_sigmasNL,u_ini_var,u,y_ini_var,data_mean,data_std,Basis_funcNARX); ([u_ini_var; y_ini_var;u]-data_mean')./data_std'; ]];
...
title ('SPC: Reference vs Closed Loop Output');
grid on
saveas(fig,'RBF-SPC\Figures\Closed Loop Output\KoopmanNARXResnet-SPC\VDP_KoopmanNARXResnet-RBF_STEPREF-SPC_output_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_Tini'+string(Tini)+'_'+Basis_funcKoopman+'.png')
...
grid on
xlabel('Iterations');
saveas(fig2,'RBF-SPC\Figures\Control Input\KoopmanNARXResnet-SPC\VDP_KoopmanNARXResnet-RBF_STEPREF-SPC_input_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_Tini'+string(Tini)+'_'+Basis_funcKoopman+'.png')
...
grid on;
xlabel('Iterations');
saveas(fig3,'RBF-SPC\Figures\Tracking Error\KoopmanNARXResnet-SPC\VDP_KoopmanNARXResnet-RBF_STEPREF-SPC_Error_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_Tini'+string(Tini)+'_'+Basis_funcKoopman+'.png')

%%

% save('Mircea-Plotting/Data/u_dataMIXEDSPC_SINREF_N'+string(N)+'_n_basis'+string(n_basis)+'_Tini'+string(Tini)+'_'+Basis_func+'.mat',"uvec")
% save('Mircea-Plotting/Data/y_dataMIXEDSPC_SINREF_N'+string(N)+'_n_basis'+string(n_basis)+'_Tini'+string(Tini)+'_'+Basis_func+'.mat',"y")