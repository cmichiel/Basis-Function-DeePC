...
% 
% [~,~,V] = svd([Phi;Y]);
% PHIY_reduced = [Phi;Y]*V(:,1:120);
% Phi_app = PHIY_reduced(1:99,:); 
% Y_red = PHIY_reduced(100:109,:);
...
constraints = [constraints,  Theta*[nl_part;  ([u_ini_var; y_ini_var;u]-data_mean')./data_std'; ]+Y*gvar==y];
...
title ('SPC: Reference vs Closed Loop Output');
grid on
saveas(fig,'RBF-DeePC-R2\Figures\Closed Loop Output\KoopmanResnet-DeePC-R2\VDP_KoopmanResnet-RBF_SINREF-DeePC-R2_output_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_Tini'+string(Tini)+'_'+Basis_funcKoopman+'.png')
...
grid on
xlabel('Iterations');
saveas(fig2,'RBF-DeePC-R2\Figures\Control Input\KoopmanResnet-DeePC-R2\VDP_KoopmanResnet-RBF_SINREF-DeePC-R2_input_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_Tini'+string(Tini)+'_'+Basis_funcKoopman+'.png')
...
grid on;
xlabel('Iterations');
saveas(fig3,'RBF-DeePC-R2\Figures\Tracking Error\KoopmanResnet-DeePC-R2\VDP_KoopmanResnet-RBF_SINREF-DeePC-R2_Error_N'+string(N)+'_n_basisKoopman'+string(n_basisKoopman)+'_Tini'+string(Tini)+'_'+Basis_funcKoopman+'.png')
